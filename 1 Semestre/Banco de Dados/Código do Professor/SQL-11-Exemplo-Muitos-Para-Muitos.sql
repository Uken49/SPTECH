...
SELECT MIN(media) AS 'Menor nota', MIN(falta) AS 'Menor quantidade de faltas' FROM Aluno,Disciplina,Matricula
	WHERE ra = fkRa AND idDisciplina = fkDisciplina;
    
-- h) Exibir a maior e a menor quantidade de faltas.
SELECT MAX(media) AS 'Maior nota', MAX(falta) AS 'Maior quantidade de faltas' FROM Aluno,Disciplina,Matricula
	WHERE ra = fkRa AND idDisciplina = fkDisciplina;
...
SELECT nomeDisciplina AS 'Nome da Disciplina',MAX(media) AS 'Maior nota', MIN(media) AS 'Menor nota' FROM Aluno, Disciplina, Matricula
	WHERE ra = fkRa AND idDisciplina = fkDisciplina
		GROUP BY fkDisciplina;
    
-- k) Exibir a maior e a menor média agrupado por aluno.
SELECT nomeAluno AS 'Nome do Aluno',MAX(media) AS 'Maior nota', MIN(media) AS 'Menor nota' FROM Aluno, Disciplina, Matricula
	WHERE ra = fkRa AND idDisciplina = fkDisciplina
		GROUP BY fkRa;
        
-- l) Exibir a maior e a menor quantidade de faltas agrupado por disciplina.
SELECT nomeDisciplina AS 'Nome da Disciplina',MAX(falta) AS 'Maior quantidade de falta', MIN(falta) AS 'Menor quantidade de falta' FROM Aluno, Disciplina, Matricula
	WHERE ra = fkRa AND idDisciplina = fkDisciplina
		GROUP BY fkDisciplina;
        
-- m) Exibir a maior e a menor quantidade de faltas agrupado por aluno.
SELECT nomeAluno AS 'Nome do Aluno',MAX(falta) AS 'Maior falta', MIN(falta) AS 'Menor falta' FROM Aluno, Disciplina, Matricula
	WHERE ra = fkRa AND idDisciplina = fkDisciplina
		GROUP BY fkRa;
...
SELECT COUNT(media) AS 'Quantidade de notas' FROM Matricula;

-- q) Exibir a quantidade de médias distintas da tabela AlunoDisciplina.
SELECT DISTINCT COUNT(media) AS 'Quantidade de notas' FROM Matricula;